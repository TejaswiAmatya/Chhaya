...
function patchComment(
  ...
  return list.map((c) => {
    ...
    if (c.replies?.length)
      return { ...c, replies: patchComment(c.replies, id, updater) };
    return c;
  });
}
...
export function StoryDetail() {
  ...
  const [suneinedComments, setSuneinedComments] = useState<Set<string>>(
    new Set(),
  );

  // ── Scroll + sync state before paint (fixes wrong story flash + preserved feed scroll) ──
  useLayoutEffect(() => {
    window.scrollTo(0, 0);
  ...
  // ── Load story when not supplied via navigation state (single-story API includes isOwner) ──
  useEffect(() => {
    const passed = (location.state as { story?: Story } | null)?.story;
    ...
    fetch(`${API}/api/stories/${id}`, { credentials: "include" })
      .then((r) => r.json())
      .then((res) => {
        if (res.success) setStory(mapApiStory(res.data as ApiStory));
  ...
  async function handleCommentSunein(commentId: string) {
    ...
    setComments((prev) =>
      patchComment(prev, commentId, (c) => ({
        ...c,
        suneinCount: c.suneinCount + 1,
      })),
    );
    try {
      await fetch(`${API}/api/comments/${commentId}/sunein`, {
        method: "POST",
        credentials: "include",
      });
    } catch {}
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await fetch(`${API}/api/stories/${story.id}`, {
        method: "DELETE",
        credentials: "include",
      });
      navigate(-1);
  ...
  const isOwner =
    !!story &&
    (story.isOwner ??
      (!!user && !!story.userId && story.userId === user.userId));
    ...
    <div className="min-h-screen bg-pageBg font-sans">
      ...
      <div className="flex justify-center">
        <main className="w-full max-w-2xl bg-feedBg min-h-screen px-3 pt-3 pb-20 md:pb-6">
          ...
          {loading && (
            <div className="text-center text-textMuted text-sm py-16">
              Katha khojdai chha...
            </div>
          )}
          ...
          {notFound && (
            <div className="text-center text-textMuted text-sm py-16 space-y-2">
              ...
              <button
                ...
                className="text-himalayan underline text-xs"
              >
                Feed ma farkinus
              </button>
            </div>
            ...
            <article className="bg-pageBg rounded-xl border border-sand p-5">
              ...
              <div className="flex items-center gap-1.5 text-xs">
                {circle && (
                  <>
                    <div
                      className={`w-4 h-4 rounded-full ${circle.color} flex items-center justify-center shrink-0`}
                    >
                ...
                {story.flair && (
                  <span
                    ...
                    style={{
                      backgroundColor: story.flair.bg,
                      color: story.flair.text,
                    }}
                  >
                    {story.flair.label}
                  </span>
              ...
              <h1
                className={`font-serif font-bold text-xl text-ink leading-snug mt-3 ${translatingStory ? "opacity-50 animate-pulse" : ""}`}
              >
                {lang === "en" && translatedBody
                  ? translatedBody.length > 80
                    ? translatedBody.slice(0, 80) + "..."
                    : translatedBody
                  : story.title}
              </h1>
              <p
                className={`text-sm text-textBody leading-relaxed mt-3 whitespace-pre-wrap ${translatingStory ? "opacity-50 animate-pulse" : ""}`}
              >
                {translatingStory
                  ? "Anuvad gardai..."
                  : lang === "en" && translatedBody
              ...
              {story.tags.length > 0 && (
                <div className="flex gap-1 mt-4 flex-wrap">
                  {story.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-[9px] bg-feedBg text-textMuted rounded-full px-2 py-0.5"
                    >
                      #{tag}
                    </span>
                  ))}
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                <button
                  ...
                  className="flex items-center gap-1 bg-feedBg rounded-full px-2.5 py-1 text-xs font-semibold text-ink hover:bg-sand/50 transition-colors shrink-0"
                >
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                    <path
                      d="M7 2L12 8H2L7 2Z"
                      fill={voted ? "#C0392B" : "#9A7B5A"}
                    />
                  </svg>
                  {voteCount}
                </button>

                <span className="flex items-center gap-1 bg-feedBg rounded-full px-2.5 py-1 text-xs text-textBody shrink-0">
                ...
                <button
                  ...
                  type="button"
                  className={`relative overflow-hidden rounded-full px-3 py-1 text-[10px] font-semibold flex items-center gap-1 transition-colors shadow-sm border border-himalayan/20 shrink-0 min-w-0 ${listened ? "bg-himalayan/85 text-pageBg" : "bg-himalayan text-pageBg hover:bg-himalayan/90 active:bg-himalayan/95"}`}
                >
                  {ripplePos && (
                    <span
          ...
          {story && (
            <section className="mt-3 space-y-2">
                  ...
                  {c.replies && c.replies.length > 0 && (
                    <div className="ml-4 mt-1 space-y-1 border-l-2 border-sand/60 pl-3">
                      {c.replies.map((r) => (
                        <div
                          key={r.id}
                          className="bg-pageBg rounded-xl border border-sand/70 p-3"
                        >
                          <p className="text-sm text-textBody leading-relaxed">
                            {r.content}
                          </p>
              ...
              <div className="bg-pageBg rounded-xl border border-sand p-3">
                ...
                />
                {commentError && (
                  <p className="text-xs text-sindoor mt-1">{commentError}</p>
                )}
