...
export function CirclePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API}/api/circles/${slug}`, { credentials: "include" }).then(
        (r) => r.json(),
      ),
      fetch(`${API}/api/circles/${slug}/stories`, {
        credentials: "include",
      }).then((r) => r.json()),
    ])
    ...
    setStories((prev) => [
      {
        ...
        createdAt: new Date(),
        theme,
        ...(user ? { userId: user.userId, isOwner: true } : {}),
      },
      ...prev,
    ]);
  }
  ...
  const circleMetadata = circle
    ? circles.find((c) => c.id === circle.circleId)
    : null;
          ...
          <div className="bg-pageBg rounded-2xl border border-sand p-4 mb-3">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                {circleMetadata && (
                  <div
                    className={`w-10 h-10 rounded-full ${circleMetadata.color} flex items-center justify-center shrink-0`}
                  >
                    <span className="text-white text-base font-bold font-serif">
                      {circleMetadata.initial}
                    </span>
                  </div>
                )}
                <div>
                  ...
                  <p className="text-[10px] text-textMuted mt-0.5">
                    {memberCount} {lang === "en" ? "members" : "sadasya"} · #
                    {circle.themeAffinity}
                  </p>
                </div>
              </div>
