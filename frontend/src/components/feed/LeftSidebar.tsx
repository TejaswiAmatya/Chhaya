...
export function LeftSidebar() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/circles/joined`, { credentials: "include" })
      .then((r) => r.json())
      .then((res) => {
        if (res.success) setJoinedIds(res.data);
      })
      .catch(() => {});
  }, [user]);
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: "🔥",
      active: isTrending,
      onClick: () => navigate("/feed?trending=true"),
    },
    {
      label: "अन्वेषण",
      labelEn: "Explore",
  ...
  return (
    <nav className="p-3 text-[13px] font-sans">
      ...
      {/* Circles */}
      <p className="text-[9px] tracking-widest uppercase text-textMuted px-2 mb-2">
        {lang === "en" ? "Circles" : "सर्कल"}
      </p>
      {circles.map((c) => (
        <Link
          key={c.id}
        ...
        >
          <div
            className={`w-4 h-4 rounded-full ${c.color} flex items-center justify-center shrink-0`}
          >
