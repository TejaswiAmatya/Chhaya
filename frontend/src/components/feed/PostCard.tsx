...
export function PostCard({
  ...
  const displayBody =
    lang === "en" ? (translatedBody ?? story.body) : story.body;
  const displayTitle =
  ...
  const isOwner =
    story.isOwner ?? (!!user && !!story.userId && story.userId === user.userId);
  ...
  return (
    <article className="bg-pageBg rounded-xl border border-sand p-3 hover:border-textMuted transition-colors duration-150">
      ...
      <Link to={`/feed/${story.id}`} state={{ story }} className="block group">
        <h3
          className={`font-serif font-bold text-[15px] text-ink leading-tight mt-1.5 line-clamp-2 group-hover:text-sindoor transition-colors ${translating ? "animate-pulse opacity-60" : ""}`}
        >
          {displayTitle}
        </h3>
        <p
          className={`text-xs text-textBody leading-relaxed mt-1 line-clamp-3 ${translating ? "animate-pulse opacity-60" : ""}`}
        >
      ...
      {/* Action bar — do not use preventDefault here; it bubbles from Links and blocks navigation */}
